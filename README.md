...
<img src="https://raw.githubusercontent.com/hoizi89/power-origin-card/main/docs/flat.png" alt="A column lying flat as a bar on the ring's middle line, two bars one under the other, and the large size on the right" width="700">

Top: the night as a column, in blocks and as one body — the second with the battery's time in the centre, so the column keeps only the gap — and the battery as a store with the sunrise level dashed. Bottom: the scale under the ring, the three devices drawing most, and a roof column alone. A column can also lie flat (`ring.meter_layout: flat`): a bar on the ring’s middle line, a second one under it, and `ring.size` scales bar and ring together. The ring’s centre can do the same with `center_dark: runtime` — how long the battery lasts. The battery block can mark where the charge will stand at sunrise (`battery.sunrise_mark`): a sun under the bar at that level — and by day, with `battery.sunset_mark`, when the forecast says the battery will not fill, a moon where it will stand at sunset. It can also draw the charge as a curve under the bar (`battery.curve`) — since sunset, dashed on to sunrise — and, as its second figure, the day's flow: what went in and out, with the cycles (`battery.extra: flow`). For a panel on the wall, `night_dim` steps the card back while the sun is below the horizon; the chip stays bright.
...
**You configure nothing.** Home Assistant's Energy dashboard already lists your devices with a live power sensor each; the card follows that list, names and meters included, and a device added there is here within the hour. A list of your own under `devices.list` is kept instead; `devices.source` says which, when both are there. The card never searches your sensors on its own — that list holds phase readings, switches at 0.0 and a fitness tracker's watts per kilo, and none of those are devices.
