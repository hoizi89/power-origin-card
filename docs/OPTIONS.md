...
| `ring.flow_pv`, `ring.flow_house`, `ring.flow_grid`, `ring.flow_battery` | `none`, house `gauge`, battery `charge` | What each circle of the flow wears inside its rim. `gauge` is the live reading: PV against today's peak, the house by where its power comes from, the grid and the battery against today's most in that direction. `day` shows how far today has come, for the battery its charge; `none` leaves a plain rim. The battery alone takes `charge` too, its charge as a thick ring outside the circle, and `cells`, the same in as many cells as the battery bar draws, one per kilowatt hour. The older `flow_gauges: false` reads as `none` for all four, and the older `clock` as the clock at the house. |
...
| `ring.flow_outer` | `day` | A second ring around each circle of the flow: where things stand today. PV produced against what is still expected, the house by origin, the grid exported against imported, and the battery's charge as a thick ring of its own. `none` leaves the circles bare. The older `clock` reads as `flow_clock: house`. |
| `ring.flow_clock` | `none` | The day's clock in the flow, 24 hours in the colour of what carried each, midnight at the bottom under a moon, noon at the top under a sun, ticks at six and eighteen, and a hand for now. `house` draws it around a bigger house circle. `strip` lays the day out beneath the flow from left to right, the hours still to come as outlines where the forecast expects sun. `ring` makes the house the big ring the standard view draws, the three sources docked around it. |
| `ring.meter_side` | `left` | Which side of the ring the first column stands on. With two columns, both swap. |
| `ring.meter_layout` | `column` | `flat` lays the columns on their side as bars beside the ring, the drawing stretched to the bar: draw to the left, surplus to the right. Words drawn inside a column, such as the night column's hours, turn with it. |
