...
| `chip` | `always` | The state word in the corner: `always`, `gridfree`, `never`. Without a title it takes no row of its own: it rides in the ring's corner, or, when the day block is first on the card, in the day's corner above the paid-off share. |
...
| `ring.view` | `columns` | How the ring block is laid out. `columns` stands the columns beside the ring, as before. `corners` puts the four powers around the ring instead: PV, grid, battery and self-supplied share, each with its direction as a word. `flow` replaces the ring with the flow: the house in the middle, PV above, the grid left, the battery right, dots running the way the power goes. Nothing is written under the circles: the icon says what, the figure how much, an arrowhead on each line which way the power goes, and the rings how far. |
| `ring.flow_pv`, `ring.flow_house`, `ring.flow_grid`, `ring.flow_battery` | `none`, house `gauge`, battery `charge` | What each circle wears on its rim. `gauge` is the moment: PV against today's peak, the house by where its power comes from now, the grid and the battery against today's most in that direction. `day` is the day so far: PV produced against what is still expected, the house by where today's power came from, the grid exported against imported. `none` leaves a plain rim. The battery takes `charge` instead of `day`: its charge as a thick ring outside the circle; `cells` draws the same in as many cells as the battery bar, one per kilowatt hour. |
| `ring.flow_dots` | `true` | In the flow, dots running along the lines; still under 30 W, and still for anyone who asked their system for reduced motion. |
| `ring.flow_outer` | | Older. `clock` reads as `flow_clock: house`; anything else is ignored, each circle chooses its own ring now. |
| `ring.flow_clock` | `none` | The day's clock in the flow, 24 hours in the colour of what carried each, midnight at the bottom under a moon, noon at the top under a sun, ticks at six and eighteen, and a hand for now. `house` draws it as a ring around a bigger house circle. `strip` lays the day out beneath the flow from left to right, the hours still to come as outlines where the forecast expects sun. The older `ring` reads as `house`. |
| `ring.meter_side` | `left` | Which side of the ring the first column stands on. With two columns, both swap. |
| `ring.meter_layout` | `column` | `flat` lays the columns on their side as bars beside the ring, the drawing stretched to the bar: draw to the left, surplus to the right. The bar sits on the ring's middle line; with `columns: two` the second bar lies under the first. `ring.size` sets bar and ring together. Words drawn inside a column, such as the night column's hours, turn with it. |
...
| `battery.sunrise_mark` | `false` | At night, a sun under the bar where the charge will stand at sunrise, worked out from today's average load, while the battery carries the house. |
| `battery.wide` | `false` | The bar across the full width of the card. The second figure moves to the end of the line under the bar, the percentage to the heading. |
| `battery.sunset_mark` | as `sunrise_mark` | By day, a moon under the bar where the charge will stand at sunset, when the forecast cannot fill it. Needs `full_from: forecast`; offered beside it in the editor. Never drawn together with the sun. |
...
| `devices.merge_icons` | `true` | In the icon grid, devices with the same icon stand as one, their watts summed; hovering names them. Icons only, and not with `group: area`. |
