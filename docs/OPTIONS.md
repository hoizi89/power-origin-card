...
| `ring.flow_pv`, `ring.flow_house`, `ring.flow_grid`, `ring.flow_battery` | `gauge` | What each circle of the flow wears inside its rim. `gauge` is the live reading: PV against today's peak, the house by where its power comes from, the grid against today's most in that direction, the battery by its charge. `day` shows how far today has come, `none` leaves a plain rim. The older `flow_gauges: false` reads as `none` for all four, and the older `clock` as the clock at the house. |
...
| `ring.flow_outer` | `none` | A second ring around each circle of the flow. `day` shows how far today has come: PV produced against what is still expected, the house by origin, the grid exported against imported, the battery charged against discharged. The older `clock` reads as `flow_clock: house`. |
| `ring.flow_clock` | `none` | The day's clock in the flow, 24 hours in the colour of what carried each, midnight at the bottom, a dial at 0, 6, 12 and 18 and a hand for now. `house` draws it around a bigger house circle. `strip` lays the day out beneath the flow from left to right, the hours still to come as outlines where the forecast expects sun. `ring` makes the house the big ring the standard view draws, the three sources docked around it. |
