...
| `ring.flow_pv`, `ring.flow_house`, `ring.flow_grid`, `ring.flow_battery` | `none`, house `gauge` | What each circle of the flow wears inside its rim. `gauge` is the live reading: PV against today's peak, the house by where its power comes from, the grid and the battery against today's most in that direction. `day` shows how far today has come, for the battery its charge; `none` leaves a plain rim. The older `flow_gauges: false` reads as `none` for all four, and the older `clock` as the clock at the house. |
