...
| `ring.flow_outer` | `none` | A second ring around each circle of the flow. `clock` runs the day's 24 hours around each, one segment an hour, the current hour marked: the house by what carried it, PV, grid and battery by how much they gave. `day` shows how far today has come: PV produced against what is still expected, the house by origin, the grid exported against imported, the battery charged against discharged. |
