...
| `ring.flow_outer` | `day` | A second ring around each circle of the flow: where things stand today. PV produced against what is still expected, the house by origin, the grid exported against imported, the battery by its charge. `none` leaves the circles bare. The older `clock` reads as `flow_clock: house`. |
